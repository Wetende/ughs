<div class="card">
    ...
    <div class="card-body">
        <form action="{{route('exams.store')}}" method="POST" class="md:w-6/12">
            ...
            <x-select id="select" name="term_id" label="Select Term *"  wire:loading.attr="disabled" wire:target="term" >
                @foreach ($terms as $item)
                    <option value="{{$item['id']}}" @selected(auth()->user()->school->term->id == $item['id'])>{{$item['name']}}</option>
                @endforeach
            </x-select>
            @csrf
